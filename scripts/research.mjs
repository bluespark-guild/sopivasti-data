...
function suggestYoutubeCategory({ name, description, recent }) {
  ...
  const rules = [
    ...
    },
    {
      cat: 'ai-slop',
      patterns: [
        /\bai-?generated\b/, /\bai brings\b/, /\bai (history|facts?|stories|narrat|video|content)/,
        /\b(top|best) \d+\b/,
        /amazing facts/, /life hacks/, /unsolved myster/,
        /weird (history|facts)/, /did you know\?\?/,
        /generated by ai/, /made with ai/,
      ],
      reason: 'AI-generated content signals: explicit "AI brings/generated" mention, or templated TTS-explainer pattern (Top N, Amazing Facts, history/mystery loops).',
    },
    {
      cat: 'rage-bait',
      patterns: [
        /react(s|ing) to|tier list|hot takes?/,
        /\bvs\b.*\bvs\b/, /\bvs\b.*\$\d/, /\$\d+,?\d* vs \$\d/,
        /destroyed|exposed|owned|cringe compilation/,
        /you won.?t believe|shocking truth/,
        /last to leave|survive \d+ (days?|hours?)/,
        /\$\d+,000,?000/, /win(s|ning)? \$\d/, /for \$\d+,?\d*,?\d*/,
        /\$1 vs \$/, /trapped (on|in)|stranded (with|in)/,
        /(prank|prick|bully|punish|destroy)(ing|ed)? my (friend|brother|sister|girlfriend|boyfriend|mom|dad)/,
        /age \d+ vs age \d+/, /age \d+-\d+ race/,
      ],
      reason: 'Rage-bait / engagement-bait pattern: MrBeast-style $-amount challenge titles, "last to leave", "survive N days", reaction farms.',
    },
    {
      cat: 'onlyfans',
      patterns: [
...
async function researchYoutube(rawUrl, info) {
  ...
  if (data) {
    ...
    // Recent videos — YT migrated /videos to lockupViewModel in 2025.
    // Fall back to legacy renderers for /featured and other layouts.
    const recent = [];
    const lockupBlocks = deepFindAll(data, (o) => o && o.lockupViewModel);
    for (const block of lockupBlocks) {
      const lvm = block.lockupViewModel;
      if (lvm.contentType && lvm.contentType !== 'LOCKUP_CONTENT_TYPE_VIDEO') continue;
      const md = lvm.metadata?.lockupMetadataViewModel;
      const title = pickFirst(md?.title?.content);
      if (!title) continue;
      const thumb = pickFirst(lvm.contentImage?.thumbnailViewModel?.image?.sources?.[0]?.url);
      const parts = md?.metadata?.contentMetadataViewModel?.metadataRows?.[0]?.metadataParts;
      const views = Array.isArray(parts) ? pickFirst(parts[0]?.text?.content) : null;
      recent.push({ title, thumb: thumb ?? null, views: views ?? null });
      if (recent.length >= 3) break;
    }
    if (recent.length === 0) {
      // Legacy renderer fallback (older channels, /featured layout)
      const legacyBlocks = deepFindAll(data, (o) => o && (o.gridVideoRenderer || o.videoRenderer || o.richItemRenderer?.content?.videoRenderer));
